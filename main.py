# main.py
# 4x4x4 立体四目並べ AI（省メモリ強化・角主張優先・支え常時ケア）
# - 即勝検出を16bitマスク化（集合/リスト大量生成なし）
# - ビーム/深さ/候補束を時間＆ノード数で自動縮退（NODE_BUDGET）
# - Corner Claim Override（角主張の上書き）/ 別角優先 / ペア角最優先（先手）を強化
# - 直ブロック > 角主張 > 縦クランプ の優先順位を維持
# - z==2 の“支え”を常時ケア（ただし直ブロックや角主張を妨げない順序）
#
...
class _Dummy: pass  # for typing-less defaults
...
def _choose_by_minimax_opp_after(board: Board, me: int, moves: List[Coord2], deadline: float, bud: Budget, base_beam: int) -> Optional[Coord2]:
    ...
    best_mv = moves[0]; best_key = (999, -1)  # (worst小 / 中央寄り大)
    for (x,y) in moves:
        if _time_up(deadline): break
...
# ---- 置石カウント ----
def _placed_count(board: Board) -> int:
    c = 0
    for z in range(4):
        for y in range(4):
            for x in range(4):
...
def corner_claim_override(board: Board, me: int, last_move: Coord3, deadline: float, bud: Budget) -> Optional[Coord2]:
    ...
    early = (_placed_count(board) <= 6)
    threshold = 2 if early else 1
    return best if w <= threshold else None
...
def urgent_vertical_clamp(board: Board, me: int, deadline: float, bud: Budget) -> Optional[Coord2]:
    ...
    # ★ 追加: z==2 の“支え”抑止を常時ケア（直ブロックや角主張の後段に位置付け）
...
def choose_best(board: Board, me: int, deadline: float, last_move: Coord3, bud: Budget) -> Coord2:
    ...
    # 2.5) Corner Claim Override（角主張：先手=ペア角最優先 / 後手=別角）
