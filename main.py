# main.py
# 4x4x4 立体四目並べ AI（相手フォーカス列クランプ + 脅威限定読み・自動反復）
# 10秒制限向け：読みは脅威集合に限定し、相手が列集中する局面では深さを+1拡張
...
def urgent_vertical_clamp(board: Board, me: int) -> Optional[Coord2]:
    """
    相手が集中している列を優先して、z==2 / z==1 を早めにクランプ。
    安全フィルタを通ったものだけ採用。
    """
    moves = valid_xy_moves(board)
    ...
    prio.sort(reverse=True)  # (種別, フォーカス補正, 中央寄り) で降順
    return prio[0][3]
...
def _tactical_candidates(board: Board, turn: int, me: int) -> List[Coord2]:
    """脅威集合（即勝ち / ブロック / 中央2-3層 / 逆ミッキーサイド / 辺の2-3層）に絞る。"""
    moves = valid_xy_moves(board)
    if not moves: return []
    you = 3 - turn
...
def choose_best(board: Board, me: int, deadline: float) -> Coord2:
    ...
    # 2.4) 逆ミッキー“完成”→サイド直行（危険手でないことを確認）
    ...
    # 2.5) 相手フォーカス列のクランプ（新規・早め）
    mv_clamp = urgent_vertical_clamp(board, me)
    if mv_clamp is not None:
        return mv_clamp

    # 2.6) 脅威だけ浅読み（相手フォーカスあれば深さ+1の反復）
    ...
    # 5) 辺（2〜3層 & 行/列角に自石）→ 側面ターゲット形最大化（安全フィルタ）
